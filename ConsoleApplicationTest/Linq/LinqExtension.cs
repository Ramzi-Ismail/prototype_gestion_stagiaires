...
namespace LinqExtension
{
    public static class Extensions 
    {
        ...
        private static Expression<Func<T, bool>> BuildPredicate<T>(string member, object value)
        {
            ...
            {
                PropertyTypeFullName = type.PropertyType.FullName;
                ...
                if (type.ToString().ToUpper().Contains(column.ToUpper()))
                {
                        ...
                        else
                        {
                            if (property.PropertyType.ToString().Contains("Int32"))
                             ...
                             else
                             {
                                ...
                                PropertyType = GetTypeFromString(PropertyTypeName + ", Entites");
                             }
                        }
                    }
                    else
                    {
                        ...
                        // Dans le cas d'un objet : BaseEntiry
                        if(PropertyType == null)
                        {
                            object obj;
                            var u = Type.GetType(PropertyTypeFullName + ", Entites");
                    ...
                    default:
                        {
                            ...
                            #region traitement des objet de Type BaseEntiry



                            Expression Out = Expression.Equal(
                                 Expression.Call(  
                                body,
                                "Id", null)
                                , Expression.Constant(Convert.ToInt32(value), value.GetType()));
